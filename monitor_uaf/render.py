...
function openProject(bulletin){current=projects.find(p=>p.bulletin===bulletin);if(!current)return;document.getElementById('dBulletin').textContent=`Boletín ${current.bulletin}`;document.getElementById('dTitle').textContent=current.title||'Título pendiente';document.getElementById('dState').textContent=[current.stage,current.state,current.commission].filter(Boolean).join(' · ')||'Estado pendiente de verificación';document.getElementById('dMeta').innerHTML=`<span>${current.lifecycle_status||'En tramitación'}</span><span>${current.relevance_label||''}</span><span>Prioridad ${current.priority||''}</span><span>${current.priority_score||0}/100</span><span>Avance ${current.probability||0}%</span>`;document.getElementById('dBody').innerHTML=`<div class="section"><h4>Vigencia legislativa</h4><div class="callout"><b>${esc(current.lifecycle_status||'En tramitación')}</b><br>${esc(current.lifecycle_reason||'Vigencia comprobada mediante fuentes oficiales.')}</div></div><div class="section"><h4>Lectura estratégica</h4><div class="callout">${esc(current.analysis_summary||'Análisis pendiente de completar con las fuentes oficiales.')}</div></div><div class="section"><h4>Dimensiones de impacto</h4><div class="impactgrid">${(current.top_impacts||[]).map(x=>`<div class="impact"><b>${esc(x.name)} · ${x.level}/5</b><small>${esc(x.recommendation||'')}</small></div>`).join('')||'<div class="empty">Sin dimensiones clasificadas.</div>'}</div></div><div class="section"><h4>Decisiones sugeridas</h4><ol>${(current.decisions||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ol></div><div class="section"><h4>Último movimiento</h4><div class="callout">${esc(current.latest_movement_date||'Fecha no disponible')} · ${esc(current.latest_movement||current.state||'Sin movimiento verificado')}</div></div><div class="section"><h4>Fuentes y auditoría</h4><ul>${(current.source_urls||[]).map(u=>`<li><a href="${esc(u)}" target="_blank" rel="noopener">${esc(u)}</a></li>`).join('')}</ul><div style="font-size:10px;color:#6b7d8d">Detectado mediante: ${esc((current.discovered_from||[]).join(' · '))}</div></div>`;document.getElementById('modal').classList.add('open');document.body.style.overflow='hidden'}
